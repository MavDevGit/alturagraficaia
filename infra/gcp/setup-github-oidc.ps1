...
gcloud iam workload-identity-pools describe $Pool --location=global --project=$ProjectId 2>$null | Out-Null
...
gcloud iam workload-identity-pools providers describe $Provider --workload-identity-pool=$Pool --location=global --project=$ProjectId 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {

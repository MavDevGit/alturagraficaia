...
function Ensure-ServiceAccount([string]$Id, [string]$DisplayName) {
  gcloud iam service-accounts describe "$Id@$ProjectId.iam.gserviceaccount.com" --project $ProjectId 2>$null | Out-Null
...
gcloud storage buckets describe "gs://$MediaBucketName" 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
...
gcloud storage buckets describe "gs://$BackupBucketName" 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
...
gcloud artifacts repositories describe altura --location=$Region --project=$ProjectId 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
...
gcloud tasks queues describe altura-image-finalize --location=$Region --project=$ProjectId 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
...
gcloud beta services identity create --service=cloudtasks.googleapis.com --project=$ProjectId | Out-Null
Assert-Gcloud 'crear identidad administrada de Cloud Tasks'
$TasksServiceAgent = "service-$ProjectNumber@gcp-sa-cloudtasks.iam.gserviceaccount.com"
gcloud iam service-accounts add-iam-policy-binding $TasksInvokerSa --project=$ProjectId --member="serviceAccount:$TasksServiceAgent" --role=roles/iam.serviceAccountTokenCreator | Out-Null
Assert-Gcloud 'autorizar OIDC de Cloud Tasks'
